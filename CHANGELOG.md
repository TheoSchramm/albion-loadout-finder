...
- Each city in the "Market city" filter now shows its own in-game crest icon (Brecilien,
  which has no crest artwork of its own, gets a neutral shield icon instead); Market city
  is now a custom dropdown like Region/Language, since no browser can show an image inside
  a native `<option>`.
